...
// ============ TUS CLASES ORIGINALES (COPIADAS) ============
class Helice {
...
class TrendeAterrizaje {
    ...
    public ToString() { return `Tren: ${this.numNeumaticos} neumáticos`; }
    public toJSON() { return { numNeumaticos: this.numNeumaticos, numAmortiguadores: this.numAmortiguadores, fijoRetractil: this.fijoRetractil }; }
}
...
class Cubierta {
    ...
    public constructor(a: boolean, b: boolean, c: boolean, d: number, e: number) {
        this.cabinaTripulacion = a; this.cabinaVuelo = b; this.sistemaEmergencia = c;
        this.numTanquesCombustible = d; this.numPuertasSalidas = e;
    }
    public ToString() { return `Cubierta: ${this.numTanquesCombustible} tanques`; }
    public toJSON() { return { cabinaTripulacion: this.cabinaTripulacion, cabinaVuelo: this.cabinaVuelo, sistemaEmergencia: this.sistemaEmergencia, numTanquesCombustible: this.numTanquesCombustible, numPuertasSalidas: this.numPuertasSalidas }; }
}

class Alas {
    private numAlasFrente: number;
    private numAlasCola: number;
    public constructor(frente: number, cola: number) { this.numAlasFrente = frente; this.numAlasCola = cola; }
...
// CREATE
function crearAeroplano(modelo: string, numHelices: number, numNeumaticos: number, numAmortiguadores: number, fijoRetractil: boolean, alasFrente: number, alasCola: number, cabinaTripulacion: boolean, cabinaVuelo: boolean, sistemaEmergencia: boolean, tanques: number, puertas: number): Aeroplano {
...
// ✅ IMPORTANTE: Ruta para servir el index.html
app.get('/', (req, res) => {
...
app.post('/api/aeroplanos', (req, res) => {
    ...
    const nuevo = crearAeroplano(modelo, numHelices, numNeumaticos, numAmortiguadores, fijoRetractil, alasFrente, alasCola, cabinaTripulacion, cabinaVuelo, sistemaEmergencia, tanquesCombustible, puertasSalida);
